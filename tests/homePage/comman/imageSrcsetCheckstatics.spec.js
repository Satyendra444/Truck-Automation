...
//   test(`Validate images on ${path}`, async ({ page }) => {
...
//     // Rule 1: Invalid srcset (/_next/image)
//     const invalidSrcset = images.filter((img) =>
//       img.srcset.startsWith("/_next/image")
//     );

//     // Rule 2: Invalid src (not in allowed domains)
//     const allowedDomains = [
//       "https://images.91trucks.com/",
//       "https://img.youtube.com/",
//     ];
//     const invalidSrc = images.filter(
//       (img) => !allowedDomains.some((domain) => img.src.startsWith(domain))
//     );

//     // Logging for Rule 1
//     if (invalidSrcset.length > 0) {
//       console.log(`\n❌ Page failed (Invalid srcset): ${url}`);
//       invalidSrcset.forEach((img, i) => {
//         console.log(
...
//       console.log(`\n❌ Page failed (Invalid src domain): ${url}`);
//       invalidSrc.forEach((img, i) => {
//         console.log(
//           `#${i + 1} | Alt: "${img.alt}" | Src: ${img.src} | Srcset: ${img.srcset}`
//         );
//       });
...
//     // Expectations
//     expect(
//       invalidSrcset.length,
//       `❌ ${url} has ${invalidSrcset.length} images with srcset starting with '/_next/image'`
//     ).toBe(0);

//     expect(
//       invalidSrc.length,
//       `❌ ${url} has ${invalidSrc.length} images with src not starting with allowed domains`
//     ).toBe(0);
//   });
// }